...
Lee semi_CSC.csv Y snippet_semi.js TAL COMO ESTÁN ESCRITOS, verifica que
...
def leer_snippet():
    js = open(os.path.join(AQUI, "snippet_semi.js"), encoding="utf-8").read()
...
def main():
    ...
    ev = json.load(open(os.path.join(AQUI, "semi_odds_snapshot.json")))
    M90, lam = [], []
    for e in ev:
    ...
    for seed in (201, 202, 203, 204):
        ...
        our = np.array([ours_pts[i] + pts(picks[int(k)][0], gh[0], ga[0])
                        + pts(picks[int(k)][1], gh[1], ga[1])
