...
def main(argv=None):
    ...
    snip = ('(function(){\n'
            ...
            '    /* ORDEN REAL del form (bug -$4.56M 19-jul): si la VISITA aparece antes que\n'
            '       el LOCAL en el título, se invierten los goles para respetar la semántica */\n'
            '    var tl=k(lab.textContent||"");var pL=9e9,pV=9e9,i2;\n'
            '    for(i2=0;i2<p.aL.length;i2++){var q=tl.indexOf(p.aL[i2]);if(q>=0&&q<pL)pL=q;}\n'
            '    for(i2=0;i2<p.aV.length;i2++){var q2=tl.indexOf(p.aV[i2]);if(q2>=0&&q2<pV)pV=q2;}\n'
            '    var swap=(pV<pL);if(swap){var tmp=gl;gl=gv;gv=tmp;}\n'
            '    setVal(cand[0],gl);setVal(cand[1],gv);used.push(cand[0],cand[1]);ok++;\n'
            '    log.push((swap?(p.V+" "+gl+"-"+gv+" "+p.L+"  [orden del form INVERTIDO, goles ajustados]"):(p.L+" "+gl+"-"+gv+" "+p.V)));\n'
            '  });\n'
